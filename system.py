...
from fns import _subplot, result_as_txt, get_GB_demand, offset, read_analysis_from_file
...
class ElectricitySystem:
    ...
    def scale_generation_tic(self, tic):
        ...
        for i in range(len(self.gen_list)):
            gen_cap.append(
                self.gen_list[i].scaled_installed_capacity
                * tic
    ...
    def get_reliability(
        self, start_up_time=0, return_output=False, return_soc=False, strategy="ordered"
    ):
        """
        == description ==
    ...
    def cost(self, x, maxcapacity=1e7, preoptimised=False):
        """
        == description ==
        Calculates the total system cost for a given set of generation and
    ...
    def analyse(self, x, filename="log/system_analysis.txt", maxcapacity=1e7):
        """
        == description ==
        ...
        None
        """
        c = self.cost(x, maxcapacity=maxcapacity)
        f = open(filename, "w")
        ...
        for i in range(len(self.storage.units)):
            f.write(
                self.storage.units[i].name
                + ": "
                + str(self.storage.units[i].capacity * 1e-6)
                + " TWh\n"
            )
        ...
        for i in range(self.n_storage):
            ...
            f.write(str(use[1][i] / n_years * 1e-6) + " TWh/yr out (grid side)\n")
            cycles = (
                use[1][i]
                * 100
                / (
                    self.storage.units[i].eff_out
                    * n_years
        ...
        f.write("-------------------\n")
        f.write(f"Reliability:{self.storage.actual_reliability}\n")
        f.write(f"-----------------------------------------")
        ...
        f.write(
            "Total Supply: "
            + str(
                ((sum(self.surplus) + sum(self.demand)) * 1e-6) / (self.t_res * n_years)
            )
            + " TWh/yr\n"
        )
        f.write("Curtailment: " + str(curt * 1e-6) + " TWh/yr\n")
        f.write("---------------\n")
        f.write("Efficiency Losses:\n")
        for i in range(len(self.stor_list)):
            f.write(
                f"\t {self.storage.units[i].name}:{self.storage.units[i].efficiencylosses * 1e-6 / n_years} TWh/yr\n"
            )
        f.write(
            f"Total shortfalls: {self.storage.energy_shortfalls*1e-6/(n_years)} TWh/yr\n"
        )
        f.write("---------------\n")
        ...
        for i in range(len(self.gen_list)):
            f.write(
                self.gen_list[i].name
                + ": £"
                + str(1e-9 * self.gen_list[i].get_cost())
                + " bn/yr\n"
            )
        for i in range(self.n_storage):
            f.write(
                self.storage.units[i].name
                + ": £"
                + str(1e-9 * self.storage.units[i].get_cost())
                + " bn/yr\n"
            )
        f.close()
        storagesizelist = [
            self.storage.units[i].capacity for i in range(self.n_storage)
        ]
        reliability = self.storage.actual_reliability
        return storagesizelist, reliability
    ...
    def optimise_total_installed_capacity(self, tic, stor_cap):
        ...
        res = minimize(
            self.cost_fixed_gen_and_stor_ratios,
            [tic],
            bounds=bounds,
            jac="2-point",
        ...
        for i in range(number_test_points):
            ...
            if max(self.max_gen_cap) < np.inf:
                for j in range(len(self.min_gen_cap)):
                    gen_cap.append(
                        (
                            x[i][j] * (self.max_gen_cap[j] - self.min_gen_cap[j])
                            + self.min_gen_cap[j]
                        )
                    )
            else:
            ...
            for j in range(len(gen_cap)):
                ...
                if gen_cap[j] < self.min_gen_cap[j] or gen_cap[j] > self.max_gen_cap[j]:
                    violation = True
    ...
    def optimise_fixed_gen_ratio(self, installed_gen, x0):
        ...
        bounds = Bounds(
            [0.85] + [0.0] * (self.n_storage - 1), [1.15] + [1.0] * (self.n_storage - 1)
        )

        if self.n_storage > 2:
            # constraint to ensure that storage ratios add up to less than 1
            linear_constraint = LinearConstraint(
                [0] + [1.0] * (self.n_storage - 1), [0], [1]
            )
    ...
    def optimise_storage_ratio(self, gen_cap, stor_cap):
        ...
        if self.n_storage > 2:
            # constraint to ensure that storage ratios add up to less than 1
            linear_constraint = LinearConstraint([1.0] * (self.n_storage - 1), [0], [1])
            constraints = [linear_constraint]
        else:
            constraints = ()
    ...
    def optimise(
        ...
        if stor_cap is None:
            print(
                "Specifying an initial relative storage capacity will "
        ...
        if gen_cap is None:
            ...
            gen_cap, cost = self.lhs_generation(
                tic0, number_test_points=15, stor_cap=stor_cap
            )
        ...
        if self.n_storage > 1:
            ...
            print(
                "Searching for an optimal storage ratio for the given generation capacities..."
            )
        ...
        print(
            "Optimising the total installed capacities of generation and storage (ratios fixed)..."
        )
        tic, cost = self.optimise_total_installed_capacity(sum(gen_cap), stor_cap)
        ...
        if analyse is True:
            self.analyse(list(gen_cap) + list(stor_cap), filename="log/opt_results.txt")
    ...
    def sensitivity_analysis(
        ...
        for mf in mult_facts:
            ...
            _x, cost = self.optimise(
                stor_cap=stor_cap,
                gen_cap=gen_cap,
                tic0=tic0,
                min_gen_cap=min_gen_cap,
                analyse=False,
    ...
    def new_analyse(self, filename="log/system_analysis.txt"):
        ...
        for i in range(len(self.gen_list)):
            f.write(
                self.gen_list[i].name
                + ": "
                + str(int(self.gen_list[i].total_installed_capacity) * 1e-3)
                + " GW\n\n"
            )
        #        f.write('\n>>TOTAL: '+str(sum(x[:len(self.gen_list)]))+' GW\n\n')
        f.write("------------------\n")
        ...
        for i in range(len(self.stor_list)):
            f.write(
                self.stor_list[i].name
                + ": "
                + str(int(self.stor_list[i].capacity * 1e-2) * 1e-4)
                + " TWh\n"
            )
        ...
        f.write(
            ...
            + str(
                int(
                    sum(self.stor_list[i].capacity for i in range(len(self.stor_list)))
                    * 1e-2
                )
                * 1e-4
            )
        ...
        for i in range(self.aggEV_list.n_assets):
            f.write(
                self.aggEV_list.assets[i].name
                + ": #V2G: "
                + str(
                    int(
                        self.aggEV_list.assets[i].chargertype[0]
                        * self.aggEV_list.assets[i].number
                    )
                )
                + ", #Smart: "
                + str(
                    int(
                        self.aggEV_list.assets[i].chargertype[1]
                        * self.aggEV_list.assets[i].number
                    )
                )
                + "\n"
        ...
        for i in range(self.n_storage):
            ...
            f.write(
                str(int(self.stor_list[i].en_in * 1e-3 / n_years) * 1e-3)
                + " TWh/yr in (grid side)\n"
            )
            f.write(
                str(-int(self.stor_list[i].en_out * 1e-3 / n_years) * 1e-3)
        ...
        for i in range(self.aggEV_list.n_assets):
            ...
            f.write(
                "V2G: "
                + str(int(self.aggEV_list.assets[i].V2G_en_in * 1e-3) * 1e-3)
                + " TWh/yr in (grid side)\n"
            )
            f.write(
                "V2G: "
                + str(int(self.aggEV_list.assets[i].V2G_en_out * 1e-3) * 1e-3)
                + " TWh/yr out (grid side)\n\n"
            )
            f.write(
                "Smart: "
                + str(int(self.aggEV_list.assets[i].Smart_en_in * 1e-3) * 1e-3)
                + " TWh/yr in (grid side)\n\n"
            )
        ...
        f.write("-------------------\n\n")
        f.write(
            "Total Passive Demand: "
            + str(int(sum(self.demand) * 1e-3 / (self.t_res * n_years)) * 1e-3)
            + " TWh/yr\n"
        )
        f.write(
            "Total EV Driving Demand: "
            + str(
                int(self.aggEV_list.driving_energy * 1e-3 / (self.t_res * n_years))
                * 1e-3
            )
            + " TWh/yr\n"
        )
        ...
        f.write(
            "Curtailment: "
            + str(int(sum(self.aggEV_list.Shed * 1e-3) / n_years) * 1e-3)
            + " TWh/yr\n\n"
        )
        ...
        for i in range(len(self.gen_list)):
            f.write(
                ...
                + str(
                    int(
                        1e-6
                        * self.gen_list[i].total_installed_capacity
                        * self.gen_list[i].fixed_cost
                    )
                    * 1e-3
                )
                + " bn/yr\n"
        ...
        for i in range(len(self.stor_list)):
            f.write(
                ...
                + str(
                    int(
                        1e-6 * self.stor_list[i].capacity * self.stor_list[i].fixed_cost
                    )
                    * 1e-3
                )
                + " bn/yr\n"
    ...
    def plot_timeseries(self, start=0, end=-1):
        ...
        if self.aggEV_list.Pfos.shape == ():
            print(
                "Charging timeseries not avaialable, try running MultipleAggregatedEVs.optimise_charger_type()."
            )
        else:
            if end <= 0:
            ...
            ax.plot(
                ...
                self.aggEV_list.Pfos[start:timehorizon],
                color="tab:red",
                label="FF Power",
            )
            ax.plot(
                range(start, timehorizon),
                self.aggEV_list.Shed[start:timehorizon],
                color="tab:blue",
                label="Renewable Shed",
            )
            ax.plot(
                range(start, timehorizon),
                self.aggEV_list.surplus[start:timehorizon],
                color="tab:orange",
                label="Surplus",
            )
            ...
            for g in range(len(self.gen_list)):
                ax.plot(
                    range(start, timehorizon),
                    self.gen_list[g].power_out[start:timehorizon],
                    color=colours[g],
                    label="Gen " + self.gen_list[g].name,
                )
            ax.plot(
                range(start, timehorizon),
                self.demand[start:timehorizon],
                color="tab:orange",
                label="Demand",
            )
...
class DispatchableOutput(ElectricitySystem):
    ...
    def plot_reliability_cost_curve(
        self,
        target_load_factor,
        min_installed=0,
        max_installed=100,
...
class CostOptimisation(ElectricitySystem):
    ...
    def cost_generation_only(self, gen_cap):
        ...
        self.update_surplus()
        sc = self.storage.size_storage(
            self.surplus, self.reliability, start_up_time=self.start_up_time
        )
        if sc == np.inf:
            return np.inf
        else:
    ...
    def optimise_generation(
        self, number_test_points=20, stor_cap=None, refine=False, x0=None
    ):
        if stor_cap is None:
            if self.n_storage == 1:
                stor_cap = []
        ...
        if refine is True:
            ...
            res = minimize(self.cost_generation_only, x0, bounds=bounds, tol=1e-3)
            best = res.x
            lwst = res.fun
    ...
    def optimise0(self, stor_cap=None):
        ...
        gen_cap, cost = self.optimise_generation(
            number_test_points=10, refine=False, stor_cap=stor_cap
        )
        stor_cap, cost = self.optimise_storage(gen_cap, stor_cap)
        gen_cap, cost = self.optimise_generation(
            stor_cap=stor_cap, refine=True, x0=gen_cap
        )
    ...
    def optimise(
        ...
        if gen_cap is None:
            gen_cap, cost = self.optimise_generation(
                number_test_points=5, refine=False, stor_cap=stor_cap
            )
        else:
        ...
        print(stor_cap)
        gen_cap, cost = self.optimise_generation(
            refine=True, x0=gen_cap, stor_cap=stor_cap
        )
        print(gen_cap)
        print(stor_cap)
    ...
    def optimise1(
        ...
        if gen_cap is None:
            gen_cap, cost = self.lhs_generation(
                tic0, number_test_points=20, stor_cap=stor_cap
            )
        else:
            cost = self.cost(gen_cap + stor_cap)
        ...
        for i in range(len(gen_cap)):
            ...
            for mf in [0.8, 0.9, 0.95, 1, 1.05, 1.1, 1.2]:
                ...
                res = (
                    ...
                    + str(mf)
                    + ":"
                    + result_as_txt(g, stor_cap, c, self.storage.capacity)
                )
                f.write(res)
                f.close()
            ...
            if stor_cap[i] < 1e-2:
                for af in [-0.5e-2, -1e-3, 0, 1e-3, 5e-3, 1e-2]:
                    ...
                    res = (
                        ...
                        + str(mf)
                        + ":"
                        + result_as_txt(gen_cap, s, c, self.storage.capacity)
                    )
                    f.write(res)
                    f.close()
    ...
    def optimise(
        ...
        if stor_cap is None:
            print(
                "Specifying an initial relative storange capacity will "
    ...
    def optimise1(
        ...
        if gen_cap is None:
            ...
            if tic0 is None:
                tic0 = 2.8e-3 * max(self.demand)  # arbitrary
            gen_cap, cost = self.lhs_generation(
                tic0, number_test_points=5, stor_cap=stor_cap
            )
        else:
            cost = self.cost(gen_cap + stor_cap)
